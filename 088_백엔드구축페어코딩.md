...
이제 본격적으로 만듭니다. 058번의 Todo API를 **모듈화**하고, TaskFlow의 핵심인 **AI 우선순위 추천** 기능을 붙입니다.
...
> 리팩터링 후에도 **기존 테스트가 통과**해야 합니다. 테스트가 "안전망" 역할을 합니다(026번).
...
> 💡 **폴백 설계**가 핵심입니다. AI 호출이 실패해도 서비스가 죽지 않고 "마감일순"으로라도 동작하게 합니다. 실무에서 외부 API 의존은 항상 실패를 대비해야 합니다.
...
> ⚠️ `.env`(진짜 키)는 **절대 커밋 금지**. `.env.example`(빈 템플릿)만 공유합니다.
...
- TaskFlow의 핵심 **AI 기능**을 폴백과 함께 구현
