# 067. apply_patch 편집 포맷 완전 정복

![그림 67-1. apply_patch 다이프 포맷](images/067_apply_patch.png)

Codex가 파일을 편집하는 핵심 도구가 **apply_patch**입니다. 그 포맷을 이해하면 Codex의 편집을 정확히 읽고, 직접 패치를 다룰 수도 있습니다.
...
- 기본적으로 변경 **위아래 3줄**의 맥락을 함께 보여 위치를 특정
- 그래도 모호하면 `@@ 함수명/클래스명` 으로 범위를 지정
- 더 모호하면 `@@`를 **여러 개 중첩**해 정확히 짚음
...
- "이 부분만 바꿔줘"처럼 **국소 편집을 요청**할 때 이 메커니즘의 혜택을 봅니다.
...
> 📷 `images/067_apply_patch.png` — 부록 프롬프트 참고
