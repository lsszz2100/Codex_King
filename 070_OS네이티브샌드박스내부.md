...
샌드박스(038번)가 "어떻게 OS 수준에서 강제되는지" 그 내부를 봅니다. Codex는 각 운영체제의 **네이티브 보안 기능**을 직접 활용합니다.
...
중요한 특성: 샌드박스는 명령 하나만이 아니라 **그 명령이 띄우는 하위 프로세스 전체**에 적용됩니다.
...
- Codex는 **OS 네이티브 보안**으로 샌드박스를 강제 (우회 어려움)
...
다음 절에서 여러 AI를 부리는 **서브에이전트 오케스트레이션**으로 넘어갑니다.
