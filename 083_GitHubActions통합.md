...
### 1) 비대화형 + 안전 경계
CI엔 사람이 없으므로 `--ask-for-approval never`가 필요합니다. 대신 **격리된 러너 + 제한된 권한**으로 4겹 안전(069번)을 유지하세요.
...
env:
  OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
```
키는 **GitHub Secrets**에 저장하고 환경변수로 주입. 절대 워크플로 파일에 직접 쓰지 마세요.

### 3) 결과 활용
- 아티팩트로 업로드
- PR에 코멘트로 게시
- 심각한 문제면 **체크 실패**로 머지 차단
...
- `codex exec`/SDK를 **GitHub Actions**에 넣어 PR·정기 자동화
- 비대화형(`never`) + **격리 러너·최소 권한**으로 안전 유지
