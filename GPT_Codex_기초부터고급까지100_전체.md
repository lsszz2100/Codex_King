...
> Codex는 각 턴(turn)마다 통신 세션을 만들고, 그 안에서 WebSocket 연결을 재사용합니다.
...
- 모델 통신은 Responses API, 턴 단위 WebSocket 세션으로 연결 재사용
- Lazy 연결 + Prewarm으로 지연 최소화
...
> 명령 목록은 버전에 따라 추가/변경됩니다. 책과 다르면 `/`로 현재 목록을, `codex --help`로 CLI 도움말을 확인하세요.
