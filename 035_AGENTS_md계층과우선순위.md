...
AGENTS.md는 한 개만 두는 게 아닙니다. **여러 계층**으로 두고, 가까운 것이 먼 것을 덮는 구조입니다. 이 메커니즘을 정확히 알면 대규모 프로젝트도 깔끔하게 통제할 수 있습니다.
...
2. **프로젝트**: 프로젝트 루트를 찾은 뒤, **루트 → 현재 작업 폴더**까지 각 디렉터리에서
...
> ⚠️ AGENTS.md를 **너무 길게** 쓰지 마세요. 길면 한도에 걸리고, 컨텍스트도 잡아먹습니다. **핵심 규칙만 간결하게**, 길면 하위 폴더로 나누는 게 좋습니다.
...
> 기본값은 **빈 목록**입니다. 즉 별도 설정 없이는 `AGENTS.md`(와 `AGENTS.override.md`)만 읽습니다. 다른 파일명을 쓰고 싶을 때만 추가하세요.
...
- AGENTS.md는 **글로벌 → 프로젝트 루트 → 하위 폴더**로 계층화
